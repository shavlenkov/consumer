...
@section('content')
    <main>
        <div class="container custom-container post-container">
            <div class="row">
            <div class="col-12">
                {{ Breadcrumbs::render('videos.show', $video) }}
...
<div class="row">
    ...
    <div class="col-12 row post-info p-0 ml-0 mr-0">
                    <span class="public-date col-md-6 col-12">
                               {{ $video->created_at->locale('uk')->format('d F Y') }}

                            </span>
    <ul class="tags-list col-md-6 col-12">
        @if($tags && $tags->count() > 0)
            @foreach($tags as $tag)
                <li><a href="{{ route('posts.by.tag', $tag->slug) }}">#{{ $tag->name }}</a></li>
            @endforeach
        @else
